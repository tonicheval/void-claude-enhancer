...
try {
    ...
    // ----------------------------------------------------
    // 1. Locate the Void Extension Folder
    // ----------------------------------------------------
    console.log(`${BOLD}[1/5] Detecting Void extension folder...${RESET}`);
    const extBase = path.join(homedir, '.void-editor', 'extensions');
    if (!fs.existsSync(extBase)) {
        throw new Error(`Void extensions directory not found at: ${extBase}`);
    }
    
    const dirs = fs.readdirSync(extBase)
        .filter(d => d.startsWith('anthropic.claude-code-') && fs.statSync(path.join(extBase, d)).isDirectory());
    
    if (dirs.length === 0) {
        throw new Error("No anthropic.claude-code extension folder found in Void extensions directory.");
    }
    
    // Sort and get the latest version
    dirs.sort((a, b) => {
        return fs.statSync(path.join(extBase, b)).mtimeMs - fs.statSync(path.join(extBase, a)).mtimeMs;
    });
    
    const extDir = path.join(extBase, dirs[0]);
    const extVersion = dirs[0];
    console.log(`  ${GREEN}✔ Found active version: ${extVersion}${RESET}`);
    ...
    // ----------------------------------------------------
    // 4. Back Up and Patch Void Extension Files
    // ----------------------------------------------------
    console.log(`\n${BOLD}[4/5] Patching Void extension code...${RESET}`);
    ...
    fs.copyFileSync(ejs, path.join(safeBackupDir, 'extension.js'));
    fs.copyFileSync(wjs, path.join(safeBackupDir, 'webview-index.js'));
    fs.copyFileSync(pkg, path.join(safeBackupDir, 'package.json'));
    
    console.log(`  ${GREEN}✔ All files patched successfully and backed up in: ${safeBackupDir}${RESET}`);
